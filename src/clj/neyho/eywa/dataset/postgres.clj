          ...
          (cond-> []
            ...
            (as-> statements
              ...
              (if (#{"user" "group" "role"} type)
                ...
                ;; If the OLD type was a reference, the column carries an FK to
                ;; user/group/role. Postgres refuses to alter the type of a column
                ;; that is under a foreign key, so drop it FIRST. Look the constraint
                ;; up by column (not by reconstructed name) so we stay correct even
                ;; when the attribute was renamed after the FK was created.
                (cond-> (vec statements)
                  (#{"user" "group" "role"} dt)
                  (conj
                   (format
                    (str "do $$ declare c text;\n"
                         "begin\n"
                         "  select conname into c from pg_constraint\n"
                         "   where conrelid = '%1$s'::regclass and contype = 'f'\n"
                         "     and conkey = (select array_agg(attnum) from pg_attribute\n"
                         "                    where attrelid = '%1$s'::regclass and attname = '%2$s');\n"
                         "  if c is not null then execute format('alter table %%I drop constraint %%I', '%1$s', c); end if;\n"
                         "end $$;")
                    old-table (normalize-name (or dn name))))
            ...
            (as-> statements
                ...
                ; -- update values with CASE statement
                ; update my_table set my_column = case ...;
                ...
                ; drop type my_enum__;
                    ...
                    (conj
                     ...
                     (format "create type %s as enum (%s)" new-enum-name (clojure.string/join "," (map #(str \' % \') (remove empty? (concat (map :name values) (vals ov))))))
                      ;; CRITICAL FIX: Convert to text BEFORE updating values
                      ;; This allows us to set any text value without enum constraints
                     (format "alter table \"%s\" alter column %s type text using %s::%s__::text"
                             old-table column column new-enum-name)
                  ...
                  ;;    END;
                     ...
                     (format "alter table \"%s\" alter column %s type %s using %s::%s"
...
(defn orphaned-attribute->drop-ddl
  ...
  (let [table-name (entity->table-name entity)
        column-name (normalize-name (:name attribute))
...
;; =============================================================================
;; RLS Compilation - Convert UUID-based config to SQL-ready schema
;; =============================================================================
...
(defn- compile-ref-condition
  ...
  (let [attr-uuid (:attribute condition)
        attr (some #(when (= attr-uuid (:euuid %)) %) (:attributes entity))]
  ...
  (let [steps (:steps condition)]
    (loop [remaining-steps steps
      ...
      (if (empty? remaining-steps)
        ...
        (let [match (cond
                      (= final-entity-uuid iu/user) :user
                      (= final-entity-uuid iu/user-group) :group
                      (= final-entity-uuid iu/user-role) :role)]
          (when match
            {:type :relation
             :match match
             :hops compiled-hops}))
    ...
    (loop [remaining-steps steps
      ...
      (if (empty? remaining-steps)
        ...
        (let [final-entity (core/get-entity model final-entity-uuid)
              final-attr (some #(when (= attr-uuid (:euuid %)) %) (:attributes final-entity))]
...
(defn- compile-rls-guard
  ...
  (let [conditions (:conditions guard)
        compiled-conditions (->> conditions
                                 ...
                                 vec)
        dropped (- (count conditions) (count compiled-conditions))]
    (when (pos? dropped)
      ;; fail-open: dropped guard/condition means its protected rows become visible
      (log/warnf "RLS: dropped %d/%d condition(s) for guard %s on entity %s[%s] - referenced attribute/relation missing or inactive in deployed model"
                 dropped (count conditions) (:id guard) (:name entity) (:euuid entity)))
     ...
     (fn [schema {:keys [euuid]
       ...
       (let [table (entity->table-name entity)
             ...
             mandatory-attributes (keep
                                   ...
                                   (:attributes entity))
              ;; Compile RLS configuration if present
             rls (compile-entity-rls model entity)
             entity-schema (cond->
                            {:table table
                             :name (:name entity)
                             :constraints (cond->
...
(comment
  ...
  (def latest-versions
    (last-deployed-version-per-dataset))
  (-> latest-versions first keys)
  (map (comp (juxt :euuid :name) :dataset) latest-versions)
  (dataset/sync-entity du/dataset-version {:euuid (:euuid version)
                                           :deployed true})

  (def kb-ticketing
    (dataset/latest-deployed-version #uuid "18df8188-8ffa-4d96-bf84-82b8b5a398b1"))

  (spit "/tmp/kb_ticketing_2.1.2_faza6" (->transit kb-ticketing))
  (core/deploy! *db* (<-transit (slurp "/tmp/kb_ticketing_2.1.2_faza6"))))
